...
with tempfile.TemporaryDirectory() as tmp:
    ...
    try:
        ...
        real_disk, prep.seconds_on_disk = prep.seconds_on_disk, lambda: topup.TARGET_SECONDS + 1
        try:
            rc = topup.main([])
            check("a full queue is left alone", calls == [] and rc == 0, str(calls))

            # the control: the same call on an empty queue must seed, otherwise
            # the check above passes on a script that never seeds at all.
            # The seeding path takes a POSIX lock, so off POSIX this is reported
            # as not run rather than as a pass it did not earn.
