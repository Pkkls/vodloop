...
echo "une demande du chat saute l attente, jamais un mur"
is "attente, et quelqu un a paye: on saute"         "$(may_jump 600 ok 1)" ""
is "mur: personne ne saute, l adresse a dit non"    "$(may_jump 600 wall 1)" mur
is "controle: un echec ordinaire ne bloque pas la demande" "$(may_jump 600 fail 1)" ""
is "controle: sans demande, on patiente"            "$(may_jump 600 ok 0)" "pas de demande"
is "controle: rien a sauter"                        "$(may_jump 0 ok 1)" "rien a sauter"

echo "une demande passe devant le tirage, pas devant le registre des echecs"
demandes=$(printf '!AAAAAAAAAAA\t100\tune\n!BBBBBBBBBBB\t100\tdeux\n')
is "jamais echouee: on la prend"                    "$(pick_requested "$demandes" "")" AAAAAAAAAAA
is "assez d echecs au compteur: la suivante"        "$(pick_requested "$demandes" AAAAAAAAAAA)" BBBBBBBBBBB
is "controle: les deux ecartees, on ne prend rien"  "$(pick_requested "$demandes" "AAAAAAAAAAA BBBBBBBBBBB")" ""

echo "la file des shorts avance meme si le premier ne vient jamais"
courts=$(printf 'CCCCCCCCCCC\tun\nDDDDDDDDDDD\tdeux\n')
is "premier id sain: on le prend"                   "$(pick_short "$courts" "")" CCCCCCCCCCC
is "premier id condamne: on prend le suivant"       "$(pick_short "$courts" CCCCCCCCCCC)" DDDDDDDDDDD
is "controle: tous condamnes, rien a prendre"       "$(pick_short "$courts" "CCCCCCCCCCC DDDDDDDDDDD")" ""
